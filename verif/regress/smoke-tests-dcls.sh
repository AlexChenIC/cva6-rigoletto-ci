...
#
# Adapted for DCLS CVA6 (cv32a60x / rv32imc) by Junchao Chen
# Based on: verif/regress/smoke-tests-cv32a65x.sh
# Changes vs cv32a65x:
#   - --target cv32a65x        → --target cv32a60x
#   - linker path cv32a65x/    → cv32a60x/
#   - Added return0.c test (verifies exit-code path)
#
# Tests included:
...
python3 cva6.py --c_tests ../tests/custom/hello_world/hello_world.c \
  --iss_yaml cva6.yaml --target cv32a60x --iss=$DV_SIMULATORS \
